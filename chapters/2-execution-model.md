...
--> The only language CPU understands is called Machine Code which interestingly always happen to have different combination of only two digits 0 and 1 representing the instruction to run the code. Kind of off(0) and on(1).

    Eg: 10 equivalent of machine code is 00001010
    
--> All programming languages have their own mechanism to translate their human readable code to machine code. The mechanism is called interpreter or compiler depending on the language.
--> Ultimately interpreter, in python's case, facilitates execution of the code by serving the human code as machine code to CPU
...
`Write code` --> python compiler reads the file from top to bottom just like a human would read, making sure the code follows python's guideline. if the code errors compiler notifies the developer to fix it. and does't proceed further
`Compile code` --> python compiles the error free code into another format called bytecode. 
`Bytecode` --> this code is OS agnostic, which means it's universal such that it can be used on any OS that has OS compatible python interpreter installed
...
`Pre compile C Function` --> The C functions then dispatch the machine code to the CPU for execution. The CPU executes the machine code and sends back the output memory location pointers
...
- During the runtime python interpreter, as the name says, interprets the bytecode that was compiled by the compiler and assigns type int to the object 10 and also makes note of the memory location of the object. Essentially when a python object is stored in memory it has all meta information like type, reference count etc about the object apart from the value itself
- Python also enables, developers to assing value or override object's method, which actually get resolved during the runtime which gives absolute flexibility but this felxibility comes with a price. The price is slower execution speed than other compile ahead on time or compile just in time interpreters or engines used by pre historic languages like C and JAVA.
...
- This is all to say that developers or hobbiest can write the code and run it with no additional cost of memory and type management. Only has to save the file in .py extension and execute with a command `python code.py` 
